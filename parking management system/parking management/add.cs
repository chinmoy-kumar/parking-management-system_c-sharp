        ...
        private void button3_Click(object sender, EventArgs e)
        {
		 ...
		 dateTimePickerStart.Text == "" || comboBox1.Text == "")
            ...
            else
            {
                ...
                string slotBoundary = comboBox1.Text.ToString().ToUpper();
                
                
                if(slotBoundary != "A" && slotBoundary != "B" && slotBoundary != "C" && slotBoundary != "D" && slotBoundary != "E") 
                {
                    MessageBox.Show("Choose valid slot!");
                    return;
                }
                ...
                cmd.Parameters.AddWithValue("@s", comboBox1.Text.ToUpper());

                cmd.Parameters.AddWithValue("@st", "not paid");

				cmd.Parameters.AddWithValue("@currentUser", username);

                string increaseSlotCountQuery = "Update ParkingSlotCounter SET VehicleCount = VehicleCount+1 Where slot = @stUpdate";
                SqlCommand slotIncreaseCmd = new SqlCommand(increaseSlotCountQuery, con);
                slotIncreaseCmd.Parameters.AddWithValue("@stUpdate", comboBox1.Text);
